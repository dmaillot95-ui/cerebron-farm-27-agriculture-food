...
def payload_for(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        elif lname in {'system','system_prompt'}: payload[name]='Evidence-grounded agriculture and food analysis. CLAIM<=EVIDENCE.'
...
record={'role':role,'model':model,'focus':focus,'inference_success':bool(ok),'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','response':text if ok else None,'error':None if ok else meta.get('error'),'meta':meta}
out=pathlib.Path('results'); out.mkdir(exist_ok=True); (out/f'{role}.json').write_text(json.dumps(record,ensure_ascii=False,indent=2)); print(json.dumps({'role':role,'inference_success':bool(ok),'model':model}))
